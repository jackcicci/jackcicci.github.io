...
function tinymce4_config($config = NULL)
{
	...
	if(file_exists($pth['folder']['userfiles'] . '_core/_split_h1only.css') && $cf['use']['h1only_pagesplitting'] == 'true')
	{
		$cssFile1 = $pth['folder']['userfiles'] . '_core/_split_h1only.css';
	}
	$cssFile2 = $pth['folder']['template'] . 'stylesheet.css';
	if(file_exists($pth['folder']['userfiles'] . '_core/_editor.css'))
	{
		$cssFile3 = $pth['folder']['userfiles'] . '_core/_editor.css';
	}
