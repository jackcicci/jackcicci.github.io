...
<ul>
<li id="pagemanager-0" title="' . $pagemanager_h[0] . '"' . ' pdattr="' . ($pd[$plugin_cf['pagemanager']['pagedata_attribute']] == '' ? '1' : $pd[$plugin_cf['pagemanager']['pagedata_attribute']]) . '"' . ($pagemanager_no_rename[0] ? ' class="pagemanager-no-rename"' : '') . '><a>'.$pagemanager_h[0].'</a><span class="pagelink" onclick="pm_pageLink(\'?' . $u[0] . '\')">&raquo;</span>';
    $stack = array();
    for ($i = 1; $i < count($h); $i++) 
	{
		$ldiff = $l[$i] - $l[$i-1];
		...
		$bo.= '<li id="pagemanager-' . $i . '"' . ' title="' . $pagemanager_h[$i] . '"' . ' pdattr="' . ($pd[$plugin_cf['pagemanager']['pagedata_attribute']] == '' ? '1' : $pd[$plugin_cf['pagemanager']['pagedata_attribute']]) . '"' . ($pagemanager_no_rename[$i] ? ' class="pagemanager-no-rename"' : '') . '><a>' . $pagemanager_h[$i] . '</a><span class="pagelink" onclick="pm_pageLink(\'?' . $u[$i] . '\')">&raquo;</span>';
	}
	$bo .= '</ul>
</div>'."\n";
