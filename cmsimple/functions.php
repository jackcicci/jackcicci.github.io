...
/*
================================================== 
This file is a part of CMSimple 4.7.2
Released: 2017-05-07
...
function newsbox($heading) 
{
	...
	for ($i = 0; $i < $cl; $i++) 
	{
		if ($h[$i] == $heading) 
		{
			...
			{
				$body = preg_replace('~<h1.*?</h1>~isu', '', $c[$i]);
			}
			else
			{
	...
	elseif ($s > 0) {
		...
		if ($tl > 1) 
		{
			for ($i = $s - 1; $i >= 0; $i--) 
			{
				if ($l[$i] < $tl) 
				{
					$t = '<span class="cmsimpleLocatorElement">' . a($i, '') . $h[$i] . '</a> &gt; </span> ' . "\n" . $t;
					$tl--;
				}
				if ($tl < 2)
					break;
		...
		return "\n" . '<span class="cmsimpleLocatorElement">' .  a(0, '') . $tx['locator']['home'] . '</a> &gt; </span> ' . "\n" . $t . '<span class="cmsimpleLocatorElement">' . str_replace('_splitToc_','',$h[$s]) . '</span> ';
		} 
		else 
		{
			return "\n" . $t . '<span class="cmsimpleLocatorElement">' . str_replace('_splitToc_','',$h[$s]) . '</span>';
